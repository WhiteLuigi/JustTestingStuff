...
class PlayState extends FlxTransitionableState
{
	...
	{
		setCountingDownFalse();
	}

	function setCountingDownFalse()
	{
		countingDown = false;
		playInst();
	}
	...
	override public function update(elapsed:Float)
	{
		...
		{
			FlxTween.tween(camFollow, {x: dad.getGraphicMidpoint().x + 150, y: dad.getGraphicMidpoint().y - 100}, 1, {ease: FlxEase.circOut});
			vocals.volume = 1;
		}

		if (playerTurn == Std.int((sectionLengths[curSection] * 8) / 2) && camFollow.x != boyfriend.getGraphicMidpoint().x - 100)
		{
			FlxTween.tween(camFollow, {x: boyfriend.getGraphicMidpoint().x - 100, y: boyfriend.getGraphicMidpoint().y - 100}, 1, {ease: FlxEase.circOut});
		}
		...
		if (curSong == 'Fresh')
		{
			...
			else if (totalBeats == 48)
			{
				gfSpeed = 1;
			}
			else if (totalBeats == 80)
			{
				gfSpeed = 2;
			}
			else if (totalBeats == 112)
			{
				gfSpeed = 1;
			}
			else if (totalBeats == 163)
			{
		...
		if (generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					else if (Math.abs(daNote.noteData) == 3)
					{
						dad.playAnim('singDOWN');
					}
					else if (Math.abs(daNote.noteData) == 4)
					{
	...
	private function keyShit():Void
	{
					...
					switch (daNote.noteData)
					{
						case 1: case 2: case 3: case 4:
							if (upP || rightP || downP || leftP)
								noteCheck(daNote.noteData == 1 ? upP : daNote.noteData == 2 ? rightP : daNote.noteData == 3 ? downP : leftP, daNote);
					}
		...
		if ((u || r || d || l) && !boyfriend.stunned && generatedMusic)
		{
			notes.forEach(function(daNote:Note)
			{
				if (daNote.canBeHit && daNote.mustPress)
				{
					...
					{
						case -1: case -2: case -3: case -4:
							if ((u && daNote.prevNote.wasGoodHit) || (r && daNote.prevNote.wasGoodHit) || (d && daNote.prevNote.wasGoodHit) || (l && daNote.prevNote.wasGoodHit))
								goodNoteHit(daNote);
					}
				}
			});
		...
		playerStrums.forEach(function(spr:FlxSprite)
		{
			...
			{
				case 1: case 2: case 3: case 4:
					if ((upP && spr.ID == 1) || (rightP && spr.ID == 2) || (downP && spr.ID == 3) || (leftP && spr.ID == 4))
						spr.animation.play('pressed');
					if ((upR && spr.ID == 1) || (rightR && spr.ID == 2) || (downR && spr.ID == 3) || (leftR && spr.ID == 4))
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (Math.abs(note.noteData) == 1)
			{
				boyfriend.playAnim('singUP');
			}
			else if (Math.abs(note.noteData) == 2)
			{
				boyfriend.playAnim('singRIGHT');
			}
			else if (Math.abs(note.noteData) == 3)
			{
				boyfriend.playAnim('singDOWN');
			}
			else if (Math.abs(note.noteData) == 4)
			{
